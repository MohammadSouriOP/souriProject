...
def upgrade():
    """Upgrade schema: rename book_id
    to id and fix foreign keys if necessary."""
    ...
    op.create_foreign_key('fk_books_id', 'books', 'other_table',
                          ['id'], ['id'])  # Adjust table & column names
...
def downgrade():
    ...
    op.create_foreign_key('fk_books_book_id', 'books', 'other_table',
                          ['book_id'], ['id'])  # Adjust table & column names
